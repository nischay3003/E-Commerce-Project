...
const AddressManagementPage: React.FC = () => {
  ...
  const handleEditSave = async (index: number) => {
    if (editingAddress && Object.values(editingAddress).every(field => field.trim())) {
      await updateUserAddress(index, editingAddress);
      handleEditCancel();
    } else {
      alert("Please fill out all address fields.");
    }
                  ...
                  {editingIndex === index && editingAddress ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2 justify-end">
                        <button onClick={() => handleEditSave(index)} className="px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md transition-colors">Save</button>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      ...
                      <div className="flex-shrink-0 flex items-center gap-4">
                        ...
                        <button onClick={() => handleRemoveAddress(index)} className="text-sm font-medium text-red-500 hover:text-red-700 transition-colors">Remove</button>
                      </div>
                    </div>
                  )}
                </li>
